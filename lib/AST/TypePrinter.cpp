...
void TypePrinter::printUPCThreadArrayBefore(const UPCThreadArrayType *T, 
                                            raw_ostream &OS) {
  IncludeStrongLifetimeRAII Strong(Policy);
  SaveAndRestore<bool> NonEmptyPH(HasEmptyPlaceHolder, false);
  printBefore(T->getElementType(), OS);
}
void TypePrinter::printUPCThreadArrayAfter(const UPCThreadArrayType *T, 
                                           raw_ostream &OS) {
  OS << '[';
...
bool Qualifiers::isEmptyWhenPrinted(const PrintingPolicy &Policy) const {
  ...
  if (hasShared() || hasStrict() || hasRelaxed())
    return false;
