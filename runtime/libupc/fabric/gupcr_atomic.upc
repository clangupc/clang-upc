/*===-- gupcr_atomic.upc - UPC Runtime Support Library -------------------===
|*
 ...
 * GUPC Libfabric UPC atomics implementation.
 *
 * All UPC atomic operations and data types, with exception of UPC_PTS,
 * are almost completely matched to the corresponding Libfabric atomics.
 * The following exceptions are made:
 *
 * UPC_SUB  Converted into Libfabric atomic add of a negative number.
 * UPC_INC  Converted into Libfabric atomic add of one.
 * UPC_DEC  Converted into Libfabric atomic add of negative one.
 *
 * UPC_PTS data type does not use Libfabric atomic operations (even though
 ...
 * disregard the phase part of the pointer and Libfabric does not have
...
/**
 * Convert UPC to Libfabric atomic data type.
 *
 * @param [in] upc_type UPC atomic data type
 * @retval Libfabric atomic data type
 */
static inline enum fi_datatype
gupcr_atomic_to_com_type (upc_type_t upc_type)
{
  switch (upc_type)
    {
    case UPC_INT:
      return UPC_ATOMIC_TO_COM_INT;
    case UPC_UINT:
      return UPC_ATOMIC_TO_COM_UINT;
    case UPC_LONG:
      return UPC_ATOMIC_TO_COM_LONG;
    case UPC_ULONG:
      return UPC_ATOMIC_TO_COM_ULONG;
    case UPC_INT32:
      return UPC_ATOMIC_TO_COM_INT32;
    case UPC_UINT32:
      return UPC_ATOMIC_TO_COM_UINT32;
    case UPC_INT64:
      return UPC_ATOMIC_TO_COM_INT64;
    case UPC_UINT64:
      return UPC_ATOMIC_TO_COM_UINT64;
    case UPC_FLOAT:
      return UPC_ATOMIC_TO_COM_FLOAT;
    case UPC_DOUBLE:
      return UPC_ATOMIC_TO_COM_DOUBLE;
    default:
...
/**
 * Convert UPC to Libfabric atomic operation.
 *
 * @param [in] upc_op UPC atomic operation
 * @retval Libfabric atomic operation
 */
static inline enum fi_op
gupcr_atomic_to_com_op (upc_op_t upc_op)
{
  switch (upc_op)
    {
    case UPC_ADD:
		    ...
		    const void *restrict operand2)
{
  ...
  /* Complete all strict operations.  Libfabric runtime allows only
  ...
  /* UPC_PTS data type does not use Libfabric atomic operations,
     ...
     container (e.g. int64) and use Libfabric atomic ops.  */
  if (ldomain->type == UPC_PTS)
	...
	case UPC_GET:
	  gupcr_atomic_get (dthread, doffset, fetch_ptr,
			    gupcr_atomic_to_com_type (ldomain->type));
	  break;
	case UPC_SET:
	...
	case UPC_MAX:
	  gupcr_atomic_op (dthread, doffset, fetch_ptr,
			   operand1, gupcr_atomic_to_com_op (op),
			   gupcr_atomic_to_com_type (ldomain->type));
	  break;
	case UPC_SUB:
	  /* As Libfabric does not have atomic subtract, UPC_SUB must be
	     converted into atomic add, UPC_ADD.  */
	  gupcr_negate_atomic_type (cvt_buf, operand1, ldomain->type);
	  gupcr_atomic_op (dthread, doffset, fetch_ptr,
			   cvt_buf, gupcr_atomic_to_com_op (UPC_ADD),
			   gupcr_atomic_to_com_type (ldomain->type));
	  break;
	case UPC_INC:
	case UPC_DEC:
	  if (op == UPC_INC)
	    ...
	    gupcr_set_optype_val (cvt_buf, ldomain->type, -1);
	  gupcr_atomic_op (dthread, doffset, fetch_ptr, cvt_buf, FI_SUM,
			   gupcr_atomic_to_com_type (ldomain->type));
	  break;
	default:
 ...
 *
 * Implementation uses native Libfabric atomic functions and the
