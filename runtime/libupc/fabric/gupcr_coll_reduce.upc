 ...
 * GUPC Libfabric reduce collectives implementation.
 *
...
/**
 * Convert from UPC reduce to Libfabric atomic operation.
 *
 * @parm [in] op UPC reduce operation
 * @retval Libfabric atomic operation
*/
enum fi_op
gupcr_fabric_reduce_op (upc_op_t op)
{
  switch (op)
    {
    case UPC_ADD:
 ...
 * - All the children perform atomic fabric reduce operations in the
 ...
 *
 * Libfabric does not define logical AND atomic operations
 ...
 *
 * Libfabric does not define logical OR atomic operations
 * and they will be executed as functions.
 */
   ...
   float (*func) (float, float), upc_flag_t sync_mode)
{
  ...
  /* Logical operations on floating point types must execute as
     functions as Libfabric does not have support for them.  */
  switch (op)
    {
    case UPC_LOGAND:
