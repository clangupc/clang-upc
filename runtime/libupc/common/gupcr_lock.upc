 ...
 * A lock is available if this pointer is NULL.  The following
 * atomic operations are used:
 * - SWAP - determine if the lock is available
 * - CSWAP - determine if the lock can be released
 *
 * The GUPC implementation of UPC locks has the following
 * characteristics:
 ...
 *   waiting thread.
 * - Atomic functions (SWAP and CSWAP) are used to
 ...
 * this is called before networking has been initialized.
 */
void
...
upc_all_lock_alloc (void)
{
  ...
  if (MYTHREAD == 0)
      ...
      else
	{
      ...
      lock->last = NULL;
      lock->owner_link = NULL;
      gupcr_broadcast_put (&lock, sizeof (lock));
    }
  else
    gupcr_broadcast_get (&lock, sizeof (lock));
  ...
  if (!compare_ok)
    {
      ...
      /* Pass ownership to the next waiting thread.  Wait until
         the link->next pointer is being set.  Use networking call to
